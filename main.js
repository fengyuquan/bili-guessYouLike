  ...
  get(bvid) {
    DB.get(bvid, (videos) => {
      if (!videos) {
        // 没有获取过推荐视频要去服务端获取
        HTTP.get(
          `https://api.bilibili.com/x/web-interface/view/detail?bvid=${bvid}&aid=&jsonp=jsonp&callback=_`
        ).then((_raw) => {
          ...
          try {
            ...
            return console.error(
              `${LOG_PREFIX} 解析detail接口返回值失败：${e}`
            );
          }
          if (res.code != 0) {
            return console.error(
              `${LOG_PREFIX} detail接口的返回值不为0：${res}`
            );
          }
          // 去掉我们不需要的信息，节约存储空间..
  ...
  recommand(n) {
    DB.getUserViewHistory((vh) => {
      ...
      DB.get(keys, (recommandArray) => {
        ...
        fresh_idx++;
        HTTP.get(
          `https://api.bilibili.com/x/web-interface/index/top/feed/rcmd?y_num=4&fresh_type=3&feed_version=V11&fresh_idx_1h=${fresh_idx}&fetch_row=1&fresh_idx=${fresh_idx}&brush=2&homepage_ver=1&ps=11&outside_trigger=`
        ).then((raw) => {
          let res;
          let data = [];
          try {
...
const UI = {
  ...
  insertRecommands() {
    return new Promise((resolve, reject) => {
      ...
      if (!anchor) {
        throw new Error(
          `${LOG_PREFIX} 无法定位首屏推荐模块 <section class="bili-grid short-margin grid-anchor">`
        );
      }
  ...
  async updateRecommands(videos) {
    ...
    if (videos.length) {
      ...
      stage.innerHTML =
        '<p style="color: #777; line-height: 360px; text-align: center; width: 100%;">观看记录为空，快去看几个视频吧~</p>';
    }
  },
  // 监听来自页面的更新请求
  listen() {
